...
export default class Newscomponent extends Component {
    articles = [
            {
                ...
                "title": "Electrify America Gives Latest Report on Efforts in California",
    ...
    constructor() {
        super();
        this.state = {
            article: this.articles // Correct variable name from 'this.article' to 'this.articles'
        };
    }

    render() {
        const articleprint=(element)=> (
        <div className='newsitems' key={element.url} >
            <Newitem title={element.title} description={element.description} image={element.urlToImage} imageurl={element.url}/>
        </div>
        )
        return (
            <>
                <div className='mainbox'>
                    <h2 style={{ textAlign: "center" }}>Welcome To the News Monkey App For the Latest and Trending News</h2>
                    <div style={{display:"flex",flexWrap:"wrap"}}>
                        {this.state.article.map(articleprint)}
                    </div>
                </div>
            </>
        );
